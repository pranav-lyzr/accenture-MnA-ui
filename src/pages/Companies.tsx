...
import type { CompanyDataProps } from '../components/companies/CompanyCard';  // Import type separately
import CompanyModal from '../components/modal/CompanyModal';
import { Search, Filter } from 'lucide-react';
import { Button } from '../components/botton';
import api, { MergerSearchResponse, EnrichedCompanyData } from '../services/api';
...
const Companies = () => {
  ...
  const createCompanyDataMapping = (mergerData: any, searchData: any) => {
    ...
    const addCompanyData = (company: any) => {
      ...
      companyMap[company.name] = {
        ...
        estimated_revenue: company.estimated_revenue || existing.estimated_revenue || 'Unknown', // Changed to estimated_revenue
    ...
    [mergerData, searchData].forEach(data => {
      if (data) {
        Object.values(data).forEach((section: any) => {
          if (section.raw_response?.companies) {
            section.raw_response.companies.forEach(addCompanyData);
          }
        });
      }
    });
  ...
  useEffect(() => {
    const loadCompanies = async () => {
      ...
      // console.log('mergerResults:', mergerResults);
