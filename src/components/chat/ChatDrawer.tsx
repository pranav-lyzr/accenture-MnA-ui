...
const ChatDrawer = ({ open, onOpenChange, agentIndexes, onResponseUpdate, activeTab }: ChatDrawerProps) => {
  ...
  useEffect(() => {
    ...
    // Clean up interval when isLoading becomes false or component unmounts
    return () => {
      ...
      <div className="flex-1 overflow-y-auto py-4 px-3 space-y-4 bg-gray-50 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="max-w-[85%] p-3 rounded-lg bg-white text-gray-800 border border-gray-200 shadow-sm flex items-center space-x-2">
              <Loader2 className="w-5 h-5 animate-spin text-purple-500" />
              <div className="text-sm text-gray-700">Fetching firms{dots}</div>
            </div>
          </div>
        )}
